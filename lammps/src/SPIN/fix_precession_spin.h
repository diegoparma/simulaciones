...
class FixPrecessionSpin : public Fix {
 ...
 public:
  ...
  int zeeman_flag, stt_flag, sotdl_flag, sotfl_flag, aniso_flag, cubic_flag, hexaniso_flag;
  ...
  // spin-orbit torque calculations

  void compute_sotdl(double *, double *);
  double compute_sotdl_energy(double *);
  void compute_sotfl(double *);
  double compute_sotfl_energy(double *);
 ...
 protected:
  ...
  // SOT damping-like intensity and direction

  double sotdl_field;
  double nsotdlx, nsotdly, nsotdlz;

  // SOT field-like intensity and direction

  double sotfl_field;
  double nsotflx, nsotfly, nsotflz;
  double sotflx, sotfly, sotflz;
