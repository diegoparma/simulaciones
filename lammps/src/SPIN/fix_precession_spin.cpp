  ...
  Fix(lmp, narg, arg), emag(nullptr)
{
  ...
  sttx = stty = sttz = 0.0;
  sotdl_field = 0.0;
  nsotdlx = nsotdly = nsotdlz = 0.0;
  sotfl_field = 0.0;
  nsotflx = nsotfly = nsotflz = 0.0;
  sotflx = sotfly = sotflz = 0.0;
  Ka = 0.0;
  nax = nay = naz = 0.0;
  Kax = Kay = Kaz = 0.0;
  k1c = k2c = 0.0;
  ...
  zeeman_flag = stt_flag = sotdl_flag = sotfl_flag = aniso_flag = cubic_flag = hexaniso_flag = 0;
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"zeeman") == 0) {
      ...
      iarg += 5;
    } else if (strcmp(arg[iarg],"sot/dl") == 0) {
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix precession/spin command");
      sotdl_flag = 1;
      sotdl_field = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      nsotdlx = utils::numeric(FLERR,arg[iarg+2],false,lmp);
      nsotdly = utils::numeric(FLERR,arg[iarg+3],false,lmp);
      nsotdlz = utils::numeric(FLERR,arg[iarg+4],false,lmp);
      iarg += 5;
    } else if (strcmp(arg[iarg],"sot/fl") == 0) {
      if (iarg+4 > narg) error->all(FLERR,"Illegal fix precession/spin command");
      sotfl_flag = 1;
      sotfl_field = utils::numeric(FLERR,arg[iarg+1],false,lmp);
      nsotflx = utils::numeric(FLERR,arg[iarg+2],false,lmp);
      nsotfly = utils::numeric(FLERR,arg[iarg+3],false,lmp);
      nsotflz = utils::numeric(FLERR,arg[iarg+4],false,lmp);
      iarg += 5;
  ...
  if (sotdl_flag) {
    norm2 = nsotdlx*nsotdlx + nsotdly*nsotdly + nsotdlz*nsotdlz;
    if (norm2 == 0.0)
      error->all(FLERR,"Illegal precession/spin command");
    inorm = 1.0/sqrt(norm2);
    nsotdlx *= inorm;
  ...
  if (sotfl_flag) {
    norm2 = nsotflx*nsotflx + nsotfly*nsotfly + nsotflz*nsotflz;
    if (norm2 == 0.0)
      error->all(FLERR,"Illegal precession/spin command");
    inorm = 1.0/sqrt(norm2);
    nsotflx *= inorm;
...
void FixPrecessionSpin::post_force(int /* vflag */)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    if (mask[i] & groupbit) {
      ...
      if (sotdl_flag) {           // compute damping-like Spin Orbit Torque
        compute_sotdl(spi,fmi);
        epreci -= compute_sotdl_energy(spi);
      }

      if (sotfl_flag) {           // compute field-like Spin Orbit Torque
        compute_sotfl(fmi);
        epreci -= compute_sotfl_energy(spi);
      }
...
void FixPrecessionSpin::compute_single_precession(int i, double spi[3], double fmi[3])
{
  ...
  if (mask[i] & groupbit) {
    ...
    if (sotdl_flag) compute_sotdl(spi,fmi);
    if (sotfl_flag) compute_sotfl(fmi);
...
void FixPrecessionSpin::compute_sotdl(double spi[3], double fmi[3])
{
  const double sx = spi[0];
  const double sy = spi[1];
  const double sz = spi[2];
  const double pmsx = nsotdly*sz - nsotdlz*sy;
  const double pmsy = nsotdlz*sx - nsotdlx*sz;
  const double pmsz = nsotdlx*sy - nsotdly*sx;

  // In nve/spin, torque is computed as fm x s, so fm = -tau_dl*(p x s).
  fmi[0] -= sotdl_field*pmsx;
  fmi[1] -= sotdl_field*pmsy;
  fmi[2] -= sotdl_field*pmsz;
}

/* ---------------------------------------------------------------------- */

double FixPrecessionSpin::compute_sotdl_energy(double * /* spi */)
{
  double energy = 0.0;  // Non-conservative force
  return energy;
}
...
void FixPrecessionSpin::compute_sotfl(double fmi[3])
{
  // In nve/spin, torque is computed as fm x s, so fm = -tau_fl*p.
  fmi[0] -= sotflx;
  fmi[1] -= sotfly;
  fmi[2] -= sotflz;
}
...
double FixPrecessionSpin::compute_sotfl_energy(double * /* spi */)
{
  double energy = 0.0;  // Non-conservative force
  return energy;
}
...
void FixPrecessionSpin::set_magneticprecession()
{
  ...
  if (sotfl_flag) {
    sotflx = sotfl_field*nsotflx;
    sotfly = sotfl_field*nsotfly;
    sotflz = sotfl_field*nsotflz;
  }
